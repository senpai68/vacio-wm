...
static void readfile(const char *path, char *buf, size_t len);
...
inputdevice(struct wl_listener *listener, void *data)
{
	 ...
	 * communiciated to the client. In vacio we always have a cursor, even if
...
maximizenotify(struct wl_listener *listener, void *data)
{
	 ...
	 * client-side decorations. vacio doesn't support maximization, but
...
apply_or_test:
	...
	/* https://codeberg.org/vacio/vacio/issues/577 */
...
run(char *startup_cmd)
{
	...
	/* #wallpaper */
   {
        ...
        execl("/bin/sh", "/bin/sh", "-c",
            "swaybg -m fill -i /usr/local/share/vacio/wallpaper/sa.png",
            NULL);
        _exit(1);
		}
	}
	 ...
	 * causing vacio to freeze when a user neither closes stdin
...
setpsel(struct wl_listener *listener, void *data)
{
	 ...
	 * ignore such requests if they so choose, but in vacio we always honor them
	 */
...
setup(void)
{
	...
	status_timer = wl_event_loop_add_timer(wl_display_get_event_loop(dpy),
		updatestatus, NULL);
	wl_event_source_timer_update(status_timer, 1000);
...
spawn(const Arg *arg)
{
	if (fork() == 0) {
		...
		die("vacio: execvp %s failed:", ((char **)arg->v)[0]);
	}
}
...
/* ==============================
 * STATUS BAR - inicio
 * Aqui se configura todo lo que
 * aparece en la barra de status.
 * Edita updatestatus() para
 * cambiar lo que se muestra.
 * ============================== */
...
updatestatus(void *data)
{
	...
	int ram_mb = (int)((mem_total - mem_available) / 1024);
	...
	if (cf) {
		...
		fscanf(cf, "cpu %ld %ld %ld %ld %ld %ld %ld", &u, &n, &s, &i, &iow, &irq, &sirq);
		fclose(cf);
		long idle = i + iow;
		long total_cpu = u + n + s + i + iow + irq + sirq;
		long diff_idle = idle - prev_idle;
		long diff_total = total_cpu - prev_total;
		if (diff_total > 0)
	...
	/* Bateria */
	char bat_buf[16] = {0}, stat_buf[16] = {0};
	readfile("/sys/class/power_supply/BAT1/capacity", bat_buf, sizeof(bat_buf));
	readfile("/sys/class/power_supply/BAT1/status", stat_buf, sizeof(stat_buf));
	int bat_pct = bat_buf[0] ? atoi(bat_buf) : -1;

	/* Audio via amixer */
	int vol = -1;
	FILE *vf = popen("amixer get Master 2>/dev/null | grep -o '[0-9]*%' | head -1 | tr -d '%'", "r");
	if (vf) {
		char vbuf[8] = {0};
		if (fgets(vbuf, sizeof(vbuf), vf)) vol = atoi(vbuf);
		pclose(vf);
	}

	/* Brillo */
	char bright_buf[16] = {0}, bright_max_buf[16] = {0};
	readfile("/sys/class/backlight/intel_backlight/brightness", bright_buf, sizeof(bright_buf));
	readfile("/sys/class/backlight/intel_backlight/max_brightness", bright_max_buf, sizeof(bright_max_buf));
	int bright_pct = -1;
	if (bright_buf[0] && bright_max_buf[0]) {
		int b = atoi(bright_buf), bmax = atoi(bright_max_buf);
		if (bmax > 0) bright_pct = b * 100 / bmax;
	}

	/* Construir stext */
	char bat_str[16] = "no bat";
	if (bat_pct >= 0)
		snprintf(bat_str, sizeof(bat_str), "󰁹 %d%%", bat_pct);
	...
	char bright_str[16] = "";
	if (bright_pct >= 0)
		snprintf(bright_str, sizeof(bright_str), " 󰛨 %d%% ", bright_pct);

	snprintf(stext, sizeof(stext), "%s  %d%% 󰤇  %dMB %s %s %s",
		bright_str, cpu_pct, ram_mb, vol_str, bat_str, hora);
...
fibonacci(Monitor *m, int s)
{
	...
	nx = m->w.x + gappx;
	ny = m->w.y + gappx;
	nw = m->w.width - 2 * gappx;
	nh = m->w.height - 2 * gappx;
...
updatemons(struct wl_listener *listener, void *data)
{
	...
	/* Make sure the clients are hidden when vacio is locked */
...
xwaylandready(struct wl_listener *listener, void *data)
{
	...
	/* Set the default XWayland cursor to match the rest of vacio. */
